...
if (!filePath) {
  console.log("\x1b[31m%s\x1b[0m", "❌ Error: No file specified.");
  console.log("\nUsage: bun run cli.ts <path-to-savefile>");
  process.exit(1);
}
...
try {
  ...
  function renderSearch() {
    ...
    currentMatches = elementsList
      .filter((el) => el.text.toLowerCase().includes(cleanQuery))
    ...
    if (currentMatches.length === 0) {
      ...
      currentMatches.forEach((el, idx) => {
        ...
        const discoveryBadge = el.discovery ? " \x1b[33m[★]\x1b[0m" : "";

        process.stdout.write(
          `\n${pointer}${el.emoji || "❓"} ${textStyle}${el.text}${resetStyle}${discoveryBadge}`,
        );
      });
    }
  ...
  function displayElementDetails(element: ICElement) {
    ...
    if (element.uses.length === 0) {
      ...
      element.uses.slice(0, 15).forEach((u) => {
        console.log(
          `   ➕ with ${u.other.emoji || ""} ${u.other.text} ➔ ${u.result.emoji || ""} ${u.result.text}`,
        );
      });
